...
// ---------------------------------------------------
unsigned int LightMapEmission::loadTexture(char const* path)
{
...
LightMapEmission::LightMapEmission(Camera& camera) :
    m_camera(camera)
{
}

void LightMapEmission::Startup()
{
    ...
    m_shader = shader_builder.Vert("4.4.lighting_maps.vs").Frag("4.4.lighting_maps.fs").Build("Light Map Emission");
    ...
    emissionMap = loadTexture("assets/textures/matrix.jpg");
...
void LightMapEmission::Shutdown()
{
...
void LightMapEmission::OnUpdate(double now, double time_step)
{
    m_shader->Bind();
...
void LightMapEmission::ImGuiUpdate()
{
