...
// ---------------------------------------------------
unsigned int HDR::loadTexture(char const* path, bool gammaCorrection)
{
...
HDR::HDR(Window& window, Camera& camera):
...
void HDR::Startup()
{
    ...
    // -------------------------
    ShaderBuilder shader_builder("src/test_progs/learn_opengl/5.advanced_lighting/6.hdr/");
    m_shader = shader_builder.Vert("6.lighting.vs").Frag("6.lighting.fs").Build("hdr lighting shader");
    m_hdr_shader = shader_builder.Vert("6.hdr.vs").Frag("6.hdr.fs").Build("hdr  shader");
...
void HDR::OnUpdate(double now, double time_step)
{
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, colorBuffer);
    m_hdr_shader->SetUniform1i("hdr", hdr);
