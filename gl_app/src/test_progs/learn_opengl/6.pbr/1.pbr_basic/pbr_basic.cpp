...
PbrBasic::PbrBasic(Window& window, v2::Camera& camera) :
...
void PbrBasic::Startup()
{
  glEnable(GL_DEPTH_TEST);

  ShaderBuilder shader_builder("src/test_progs/learn_opengl/6.pbr/1.pbr_basic/");
  m_shader = shader_builder.Vert("1.1.pbr.vs").Frag("1.1.pbr.fs").Build("Basic PBR shader");
  
  m_shader->Bind();
  m_shader->SetUniform3f("albedo", 0.5f, 0.0f, 0.0f);
  m_shader->SetUniform1f("ao", 1.0f);
  m_shader->SetUniformMat4f("projection", m_camera.GetProjMatrix());
}

void PbrBasic::Shutdown()
{
  glfwTerminate();
}

void PbrBasic::OnUpdate(double now, double time_step)
{
  m_shader->Bind();
...
void PbrBasic::ImGuiUpdate()
{
}

// renders (and builds at first invocation) a sphere
// -------------------------------------------------
void PbrBasic::renderSphere()
{
  if (sphereVAO == 0)
  {
...
// ---------------------------------------------------
unsigned int PbrBasic::loadTexture(char const* path)
{
...
void PbrBasic::CheckKeys(double delta_time)
{
...
void PbrBasic::OnEvent(Event& event)
{
