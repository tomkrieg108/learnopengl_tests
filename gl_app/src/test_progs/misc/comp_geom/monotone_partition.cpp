...
namespace geom
{
	...
	static void handle_start_vertices(Vertex2dDCELWrapper& vertex
		...
		, std::map<Edge2dDCEL*, Edge2dDCELWrapper*>& edge_mapper, Polygon2dDECL* poly)
	{
	...
	void jmk::get_monotone_polygons(Polygon2dDECL* poly, std::vector<Polygon2dDECL*>& mono_polies)
	{
		...
		Point2d* sweep_point = new Point2d(vertices[0].vert->point.y, vertices[0].vert->point.x);
		...
		for (auto face_ptr : poly->face_list) {
		...
		for (auto vertices : polygon_pieces_vertices)
			mono_polies.push_back(new Polygon2dDECL(vertices));
	}
